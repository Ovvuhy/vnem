...
- `vnem_design_ambition_plan`: for UI/redesign tasks, follow explicit user style when given; otherwise adapt to business, audience, brand, content, and conversion goal, consider references when useful, and reject generic-template defaults.
- `vnem_visual_taste_audit`: audit planned/completed design work for boring/generic risk, weak hero/CTA/typography/spacing/mobile polish, ignored user style, weak brand fit, missing visual proof, and missing before/after comparison.
