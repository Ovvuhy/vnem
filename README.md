...
| UI/web quality proof | Core `vnem_build_ui_quality_plan` and `vnem_visual_proof_contract` produce plan-only visual-proof requirements for routes/components, before/after screenshots, console/network checks, accessibility checks, multiple responsive viewports, and empty/loading/error states. Tools `vnem_tools_ui_surface_review`, `vnem_tools_browser_evidence_plan`, and `vnem_tools_ui_evidence_audit` safely review local UI surfaces, plan browser evidence without running hidden browser automation, and reject UI-improved/responsive/accessibility/browser-works claims when screenshot/DOM/route/render/console/network/a11y/viewport/state evidence is missing. |
