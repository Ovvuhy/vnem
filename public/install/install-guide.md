...
Generated: 2026-06-21T19:18:58.554Z
...
- MCP activation: connect the client and call `vnem_bootstrap` with the real task. Confirm the structured output includes `activation.status=active`, `read_only=true`, `precision_tools_exposed=false`, `compact_startup_contract`, task-specific `required_rules`, `recommended_vnem_calls`, `protection_needs`, `verification_contract`, `completion_audit_expectations`, and `anti_placebo_checks`.
- Agent flow: after `vnem_bootstrap`, call `vnem_compose_capability_contract` or `vnem_get_required_capabilities` for compact capability modules, use returned `vnem://...` resources/rules, run `vnem_quality_gate`, then report the bootstrap `activation_id`, capability IDs used, tools/rules used, changed files, commands/checks run, skipped checks, remaining risks, and evidence.
- Capability library: when `vnem_bootstrap` reports skill/API availability, use `vnem_library_status`, `vnem_get_required_capabilities`, `vnem_activate_capability_pack`, `vnem_apply_skill_guidance`, `vnem_build_api_integration_plan`, `vnem_get_agent_profile`, `vnem_compose_capability_contract`, `vnem_recommend_skills`, `vnem_recommend_apis`, and `vnem_review_skill_or_api` for read-only capability activation. These tools do not install skills, execute skill scripts, call APIs, request API keys, mutate files, or guarantee safety/freshness.
