...
Generated: 2026-06-21T23:20:00.150Z
...
- MCP activation: connect the client and call `vnem_bootstrap` with the real task. Confirm the structured output includes `activation.status=active`, `read_only=true`, `precision_tools_exposed=false`, `compact_startup_contract`, `missing_context`, `domain_quality_contracts`, task-specific `required_rules`, `recommended_vnem_calls`, `protection_needs`, `verification_contract`, `completion_audit_expectations`, and `anti_placebo_checks`.
- Agent flow: after `vnem_bootstrap`, call `vnem_compose_capability_contract` or `vnem_get_required_capabilities` for compact capability modules, use returned `vnem://...` resources/rules, call `vnem_protection_review` before risky filesystem/terminal/browser/GitHub/package/API/skill/modding actions, run `vnem_quality_gate`, call/apply `vnem_completion_audit` before final response, then report the bootstrap `activation_id`, capability IDs used, tools/rules used, changed files, commands/checks run, skipped checks, remaining risks, assumptions, and evidence.
- Capability library: when `vnem_bootstrap` reports skill/API availability, use `vnem_library_status`, `vnem_get_required_capabilities`, `vnem_activate_capability_pack`, `vnem_apply_skill_guidance`, `vnem_build_api_integration_plan`, `vnem_get_agent_profile`, `vnem_compose_capability_contract`, `vnem_completion_audit`, `vnem_protection_review`, `vnem_recommend_skills`, `vnem_recommend_apis`, and `vnem_review_skill_or_api` for read-only capability activation, audit, and protection review. These tools do not install skills, execute skill scripts, call APIs, request API keys, mutate files, or guarantee safety/freshness.
- Domain contracts: research/source-quality work must use current/high-quality sources where facts can change; UI/backend work needs visible user-path and visual evidence; API work needs auth/CORS/HTTPS/secret/backend handling; game/build/modding work needs specific game/version/tool/file-format context, backups/isolation where mutation is proposed, and no generic best-build claims without PvE/PvP/DLC/progression assumptions.
