...
- Launch/health: `npm run ard:dev` starts the local VNEM backend on `127.0.0.1:9099` when needed and starts/reuses the dashboard on `127.0.0.1:4174`. `npm run ard:health` prints backend URL, dashboard URL, port state, local wallet allowlist guidance, and cleanup guidance. Split commands are available as `npm run ard:backend` and `npm run ard:dashboard`. Local dashboard owner access allowlists `H62Ri1EExddxFKsLMn4nbmbxiCSxNRLtF8igPySLA23B`; production deployments still need explicit auth secrets and allowlists.
...
- Current focused test path: `npm run test:current` runs the ARD browser-pipeline smoke test, the dedicated dashboard browser-pipeline status test, and the Changes by ARD helper/dashboard tests. User-facing local steps live in `docs/local-testing.md`.
- Changes by ARD: protected implementation lane with display name `Changes by ARD` and Git branch `changes-by-ard`. `GET /api/ard/changes/status` reports branch state, `POST /api/ard/changes/preview` is dry-run only, `POST /api/ard/changes/prepare` creates a real local commit on the protected branch when the repo is clean, and `POST /api/ard/changes/push` pushes only `origin changes-by-ard` after exact confirmation. It never merges or pushes `main`.
...
- Dashboard: the top ARD view is now a canonical operator console: ARD Mission Header, ARD Control Center, Pipeline Timeline, Changes by ARD Protected Branch, Review Queue, AI Status & Public Decision Log, Findings / Evidence Explorer, System Health, Advanced / Raw Details, and Planned Features. Older raw telemetry, Builder Health, run history, and logs are collapsed under advanced details. ARD v2 states include source lanes, candidate lifecycle counts, repeated/suppressed candidates, work packages, exact files, prepared/pushed commit fields, and blocked reasons.
- ARD dogfood: `npm run ard:dogfood` runs the improved repo/local loop and proves source lanes, category diversity, external GitHub auto-triage, lifecycle memory, candidate scoring/ranking, Protection AI v2 branch eligibility, Giving AI v2 work packages, review-artifact-only external candidates, and Changes by ARD work-package preview readiness.

ARD v2.2 research categories include AI skills, MCPs, agent frameworks, coding tools, research methods, evals/benchmarks, safety/security, prompting playbooks, repo automation, documentation systems, browser automation, data/memory/retrieval, Roblox/Luau, and general devtools. Roblox/Luau is capped to one category; repeated missing-license external repos become waiting-for-evidence or review-artifact-only instead of implementable work.
