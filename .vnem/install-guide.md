...
Generated: 2026-06-22T21:46:45.545Z
...
Opt-in Tools MCP foundation for a project that should allow bounded approved file reads/searches, dry-run-first patches, allowlisted commands, approved GET/HEAD API requests, and evidence logs:
...
Tools MCP is separate from Core MCP and is not Giga MCP: browser screenshots, GitHub mutation, package installs, arbitrary shell, and unrestricted API calls are not in this foundation batch.
...
- Agent flow: call `vnem_bootstrap`, then `vnem_boost_task` for the concrete user-task workflow. It selects usable API/skill packs rather than raw records and returns a compact `tools_mcp_handoff`. Use `vnem_prepare_tools_handoff` when a standalone Tools MCP handoff is needed. Connect the separate `vnem-tools` MCP foundation only when approved actions are needed; it dry-runs first, asks permission, applies only approved path-limited patches, runs only allowlisted commands, performs only approved GET/HEAD API requests, and collects redacted evidence. Use `vnem_compose_capability_contract` only when lower-level capability IDs/details are needed. Call `vnem_protection_review` before risky filesystem/terminal/browser/GitHub/package/API/skill/modding actions; collect task-specific checks/evidence; call/apply `vnem_completion_audit`; then call `vnem_proof_trail` and include its compact proof/evidence summary in the final response.
