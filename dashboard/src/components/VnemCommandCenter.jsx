...
import { AlertTriangle, GitBranch, Play, Radio, RefreshCcw, Search, ShieldCheck } from "lucide-react";
import { normalizeRequestError } from "../lib/vnemApiClient.js";
import { Badge } from "./PipelinePrimitives.jsx";

export function VnemCommandCenter({ workStatus, telemetry, apiClient, onBranchPreview, onReviewCandidate }) {
  ...
  const primary = primaryAction({ workStatus, topCandidate, redeployMission, previewBranch, onReviewCandidate });
  ...
  return (
    <section className="command-center" aria-label="VNEM AI command center">
      ...
      <div className="command-action-row">
        <button type="button" className="primary-action" onClick={primary.onClick} disabled={!primary.enabled || ["deploying", "previewing"].includes(actionState)}>
          {actionState === "deploying" || actionState === "previewing" ? <RefreshCcw size={17} /> : primary.icon}
          {actionState === "deploying" ? "Redeploying..." : actionState === "previewing" ? "Previewing..." : primary.label}
        </button>
        ...
        <button type="button" className="secondary-action" disabled title={workStatus.giving.prepareDisabledReason}>
          <GitBranch size={16} /> Prepare branch disabled
        </button>
...
function primaryAction({ workStatus, topCandidate, redeployMission, previewBranch, onReviewCandidate }) {
  ...
  if (workStatus.giving.previewAvailable && workStatus.giving.previewStatus !== "ready") {
    return { label: "Preview branch plan", detail: "Calls the backend preview endpoint only; it does not mutate git.", enabled: true, icon: <GitBranch size={17} />, onClick: previewBranch };
  }
  if (topCandidate) {
    return { label: "Review top candidate", detail: "Open the best candidate first instead of scanning every raw finding.", enabled: true, icon: <ShieldCheck size={17} />, onClick: () => onReviewCandidate?.(topCandidate) };
  }
  return { label: "Redeploy mission", detail: "Calls the existing intelligence target endpoint and waits for real telemetry.", enabled: true, icon: <Radio size={17} />, onClick: redeployMission };
}
...
function statusSentence(workStatus) {
  ...
  if (workStatus.status.key === "branch-preview-ready") {
    return "VNEM has a backend branch preview. Prepare remains disabled until explicit confirmation UX is implemented.";
  }
