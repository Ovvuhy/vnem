...
export function scriptsForTier(tier, affected = []) {
  ...
  if (tier === "tools") return unique([...VNEM_FULL_SUITE.filter((name) => /^test:tools-|^test:permission-runtime$|^test:safety-cli$/.test(name)), "test:tools-giga-testing-ci", "test:tools-giga-browser-interaction"]);
  if (tier === "integration") return unique([...VNEM_FULL_SUITE.filter((name) => INTEGRATION_PATTERN.test(name)), "test:tools-giga-testing-ci", "test:tools-giga-browser-interaction"]);
  if (tier === "full" || tier === "ci") return VNEM_FULL_SUITE.flatMap((script) => {
    ...
    if (script === "test:tools-giga-project-automation") return [script, "test:tools-giga-testing-ci"];
