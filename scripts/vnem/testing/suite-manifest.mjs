...
export function scriptsForTier(tier, affected = []) {
  ...
  if (tier === "tools") return unique([...VNEM_FULL_SUITE.filter((name) => /^test:tools-|^test:permission-runtime$|^test:safety-cli$/.test(name)), "test:tools-giga-testing-ci"]);
  if (tier === "integration") return unique([...VNEM_FULL_SUITE.filter((name) => INTEGRATION_PATTERN.test(name)), "test:tools-giga-testing-ci"]);
  if (tier === "full" || tier === "ci") return VNEM_FULL_SUITE.flatMap((script) => {
    ...
    return script === "test:tools-giga-project-automation" ? [script, "test:tools-giga-testing-ci"] : [script];
  });
