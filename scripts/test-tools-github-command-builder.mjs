...
await withGithubMockTools({}, async ({ client, workspace, commandLog }) => {
  const providerToken = ["github", "pat", "A".repeat(24)].join("_");
  const branch = await client.callTool({ name: "vnem_tools_github_branch_create", arguments: { root: workspace, branch: "feat/autonomy-2", dry_run: false, allow_dirty: true } });
  ...
  const issueBody = `Authorization: Bearer bearer-secret-value api_key=abc123 SECRET_TOKEN_VALUE ${providerToken}`;
  const issue = await client.callTool({ name: "vnem_tools_github_issue_create", arguments: { root: workspace, title: "bug: redaction", body: issueBody, dry_run: false } });
  ...
  assert.doesNotMatch(commandLogText, /bearer-secret-value|abc123|SECRET_TOKEN_VALUE/);
  assert.equal(commandLogText.includes(providerToken), false);
