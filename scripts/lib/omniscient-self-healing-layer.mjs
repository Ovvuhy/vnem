...
import { PrecisionExecutionError, StatefulTerminalSession } from "./precision-execution-layer.mjs";
...
export class CodebaseSemanticIndex {
  ...
  async ensureFresh(options = {}) {
    ...
    if (!this.state || options.refresh || this.dirty || (await this.isStale())) {
      await this.rebuild();
    }
  }
...
export class VerificationLoopStore {
  ...
  reset(taskId = "default") {
    return this.records.delete(this.key(taskId));
  }
