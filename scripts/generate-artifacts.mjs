...
function installGuideMarkdown() {
  return [
    ...
    "- MCP activation: connect the client and call `vnem_bootstrap` with the real task. Confirm the structured output includes `activation.status=active`, `read_only=true`, `precision_tools_exposed=false`, task-specific `required_rules`, `recommended_vnem_calls`, `protection_needs`, `verification_contract`, `completion_audit_expectations`, and `anti_placebo_checks`.",
    "- Agent flow: after `vnem_bootstrap`, call the recommended next tools, use returned `vnem://...` resources/rules, run `vnem_quality_gate`, then report the bootstrap `activation_id`, tools/rules used, changed files, commands/checks run, skipped checks, remaining risks, and evidence.",
    "- Capability library: when `vnem_bootstrap` reports skill/API availability, use `vnem_library_status`, `vnem_recommend_skills`, `vnem_recommend_apis`, and `vnem_review_skill_or_api` for read-only capability discovery. These tools do not install skills, execute skill scripts, call APIs, request API keys, or guarantee safety/freshness.",
